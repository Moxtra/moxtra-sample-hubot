...
# Commands:
#   ------------------------------ Google Maps ------------------------------
#   hubot [satellite|terrain|hybrid] map me <place> - Map view of the area.
#   hubot [driving|walking|bike|biking|bicycling] directions from <place> to <place> - Navigation instructions.
...
module.exports = (robot) ->
  ...
  robot.respond /((driving|walking|bike|biking|bicycling) )?directions from (.+) to (.+)/i, (msg) ->
    ...
    key         = 'AIzaSyCWSP5-H-R3lTMIKqUvjOHFbzDV_e4Jqx0'
    ...
    robot.http(url).query(query).get()((err, res, body) ->
      ...
      # msg.send "[img=400x300]http://maps.googleapis.com/maps/api/staticmap?size=400x300&maptype=roadmap&" +
              #  "path=weight:3%7Ccolor:red%7Cenc:#{route.overview_polyline.points}&sensor=false&format=png[/img]"
      msg.send response
    )
  ...
  robot.respond /(?:(satellite|terrain|hybrid)[- ])?map( me)? (.+)/i, (msg) ->
    ...
    mapUrl   = "[img=400x300]http://maps.google.com/maps/api/staticmap?markers=" +
                location +
                "&size=500x400&maptype=" +
                mapType +
                "&sensor=false" +
                "&format=png[/img]" # So campfire knows it's an image
